...
function MenuItems() {
    ...
    async function fetchMenuItems() {
        ...
        setPageCount(data.menuItemCount/amount)
    }
                ...
                <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                    ...
                    {menuItems.map((menuItem) => (
                        <tr key={menuItem.id} className="bg-white border-b">
                            ...
                            <td className="px-6 py-4">{new Intl.NumberFormat('nl-NL', {
                                style: 'currency',
                                currency: 'EUR'
                            }).format(menuItem.price / 100)}</td>
                            <td className="px-6 py-4">
                                <img src={menuItem.imageUrl} alt={menuItem.name} className="h-16 w-16 object-cover"/>
                            </td>
                            <td className="px-6 py-4">
                                <Link to={`/admin/menuItems/${menuItem.id}/edit`}
